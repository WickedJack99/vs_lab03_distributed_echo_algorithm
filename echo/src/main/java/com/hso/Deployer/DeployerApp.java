...
public class DeployerApp extends Thread {
    ...
    private void setPathToNetworkConfigFile(Scanner inputScanner) {
        ...
        try {
            ...
            System.err.println("Error at DeployerApp:setPathToNetworkConfigFile, unable to resolve command from input scanner.");
        }
    ...
    private void setPathToJar(Scanner inputScanner) {
        ...
        try {
            ...
            System.err.println("Error at DeployerApp:setPathToJar, unable to resolve command from input scanner.");
        }
        if ((inputString != null) && (!inputString.equals(""))) {
            File f = new File(inputString);
            if(f.exists() && !f.isDirectory()) { 
    ...
    private void createProcesses() {
        try {
            ...
            String[] controllerJarArgs = {"controller", controllerJSONObject.getString("ipPort"), this.pathToNetworkConfigFile};
    ...
    private static String[] buildCommand(String jarPath, String[] args) {
        // Create the command array with "java", "-jar", jarPath, mode, ipPort, storage, controller ipPort, direct neighbors ipPort
